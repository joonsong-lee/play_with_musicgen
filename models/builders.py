...
def get_lm_model(cfg: omegaconf.DictConfig) -> LMModel:
    ...
    if cfg.lm_model in ['transformer_lm', 'transformer_lm_magnet']:
        ...
        condition_provider,dict_cfg,output_dim = get_conditioner_provider(kwargs["dim"], cfg)
        condition_provider.to(cfg.device)
        ...
        return lm_class(
            ...
            device=cfg.device,
            **kwargs
        ).to(cfg.device),dict_cfg,output_dim
    else:
...
def get_conditioner_provider(output_dim: int, cfg: omegaconf.DictConfig) -> ConditioningProvider:
    ...
    return conditioner, dict_cfg,output_dim

def get_style_provider(output_dim:int,dict_cfg:tp.Dict,device,duration,style_type:str):
    for cond, cond_cfg in dict_cfg.items():
        model_type = cond_cfg['model']
        model_args = cond_cfg[model_type]
        if model_type == 'chroma_stem':
            conditioner = style_wav_Conditioner(
                output_dim=output_dim,
                duration=duration,
                device=device,
