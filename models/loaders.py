...
def load_lm_model(file_or_url_or_id: tp.Union[Path, str], device='cpu', style_type:str = 'ratio',cache_dir: tp.Optional[str] = None):
    ...
    model,dict_cfg,output_dim = builders.get_lm_model(cfg)
    model.load_state_dict(pkg['best_state'])
    self_st = builders.get_style_provider(output_dim,dict_cfg,cfg.device,cfg.dataset.segment_duration,style_type).to(cfg.device)
    model.condition_provider.conditioners['self_st']=self_st
    model.condition_provider.conditioners['self_st'].load_state_dict(model.condition_provider.conditioners['self_wav'].state_dict(),strict=False)
    model.eval()
    model.cfg = cfg
    return model
