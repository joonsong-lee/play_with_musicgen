...
class ConditioningAttributes:
    ...
    content:tp.Dict[str, WavCondition] = field(default_factory=dict)
    style:tp.Dict[str, WavCondition] = field(default_factory=dict)
    ...
    def attributes(self):
        return {
            ...
            "content":self.wav_attributes,
            "style":self.st_attributes
    ...
    def to_flat_dict(self):
        return {
            ...
            **{f"content.{k}": v for k, v in self.content.items()},
            **{f"style.{k}": v for k, v in self.style.items()}
        }
...
class style_wav_Conditioner(WaveformConditioner):
    """Chroma conditioner based on stems.
    The ChromaStemConditioner uses DEMUCS to first filter out drums and bass, as
    the drums and bass often dominate the chroma leading to the chroma features
    not containing information about the melody.
    ...
    """
    def __init__(self, output_dim: int, sample_rate: int, n_chroma: int, radix2_exp: int,
                 duration: float, style_type:str,match_len_on_eval: bool = True, eval_wavs: tp.Optional[str] = None,
                 n_eval_wavs: int = 0, cache_path: tp.Optional[tp.Union[str, Path]] = None,
                 device: tp.Union[torch.device, str] = 'cpu',  **kwargs):
        from demucs import pretrained
        super().__init__(dim=n_chroma, output_dim=output_dim, device=device)
        ...
        if(self.style_type == 'last_step'):
            with torch.no_grad():
                self.output_proj2 = nn.Linear(self.dim,self.output_dim)
                self.output_proj2.to(self.device)
                self.output_proj2.load_state_dict(self.output_proj.state_dict())
    ...
    def _compute_wav_embedding(self, wav: torch.Tensor, sample_rate: int) -> torch.Tensor:
        """Compute wav embedding, applying stem and chroma extraction."""
        # avoid 0-size tensors when we are working with null conds
        self.chroma.argmax = False
        if wav.shape[-1] == 1:
            return self._extract_chroma(wav)
        stems = self._get_stemmed_wav(wav, sample_rate)
        #stems,db = self._get_stemmed_wav(wav, sample_rate)
        chromas = []
        for stem in stems:
            chromas.append(self._extract_chroma(stems))
        chroma = torch.empty_like(chromas[0])
        '''for wav,perc in (chromas,db):
                chroma+= wav*perc'''
        if(self.style_type == 'ratio'):
            return chroma
        elif(self.style_type == 'last_step'):
            #return chroma,chromas[db.index(max(db))]
            return chromas[0],chromas[0]
    ...
    def _get_wav_embedding(self, x: WavCondition) -> torch.Tensor:
        ...
        if (self.style_type=='ratio'):
            ...
            if sampled_wav is not None:
                chroma,sp_chroma = self._compute_wav_embedding(sampled_wav, self.sample_rate)
            elif self.cache is not None and no_undefined_paths and no_nullified_cond:
                paths = [Path(p) for p in x.path if p is not None]
                chroma,sp_chroma = self.cache.get_embed_from_cache(paths, x)
            else:
                assert all(sr == x.sample_rate[0] for sr in x.sample_rate), "All sample rates in batch should be equal."
                chroma,sp_chroma = self._compute_wav_embedding(x.wav, x.sample_rate[0])
            if self.match_len_on_eval:
                B, T, C = chroma[0].shape
                if T > self.chroma_len:
                    chroma = chroma[:, :self.chroma_len]
                    sp_chroma=sp_chroma[:,:self.chroma_len]
                    logger.debug(f"Chroma was truncated to match length! ({T} -> {chroma.shape[1]})")
                elif T < self.chroma_len:
                    n_repeat = int(math.ceil(self.chroma_len / T))
                    chroma = chroma.repeat(1, n_repeat, 1)
                    chroma = chroma[:, :self.chroma_len]
                    sp_chroma = sp_chroma.repeat(1, n_repeat, 1)
                    sp_chroma = sp_chroma[:, :self.chroma_len]
                    logger.debug(f"Chroma was repeated to match length! ({T} -> {chroma.shape[1]})")

            return chroma,sp_chroma
    ...
    def forward(self, x: WavCondition) -> ConditionType:
        ...
        if(self.style_type == 'ratio'):
            ...
            with torch.no_grad():
                embeds,sp_embeds = self._get_wav_embedding(x)
            embeds = embeds.to(self.output_proj.weight)
            embeds = self.output_proj(embeds)
            sp_embeds = sp_embeds.to(self.output_proj2.weight)
            sp_embeds = self.output_proj2(sp_embeds)
            if lengths is not None and self._use_masking:
                lengths = lengths / self._downsampling_factor()
                mask = length_to_mask(lengths, max_len=embeds.shape[1]).int()  # type: ignore
            else:
...
def dropout_condition(sample: ConditioningAttributes, condition_type: str, condition: str) -> ConditioningAttributes:
    ...
    Works in-place.
    """
    if condition_type not in ['text', 'wav', 'joint_embed','content','style']:
        raise ValueError(
            "dropout_condition got an unexpected condition type!"
            f" expected 'text', 'wav','joint_embed','content' or 'style but got '{condition_type}'"
        )
    ...
    if condition_type == 'wav':
        ...
        content_cond = sample.content[condition]
        sample.content[condition] = nullify_wav(content_cond)
    elif condition_type=='style':
        style_cond = sample.style[condition]
        sample.style[condition] = nullify_wav(style_cond)
    else:
...
class ClassifierFreeGuidanceDropout(DropoutModule):
    ...
    def forward(self, samples: tp.List[ConditioningAttributes]) -> tp.List[ConditioningAttributes]:
        ...
        samples = deepcopy(samples)
        for condition_type in ["content", "style"]:
            for sample in samples:
...
class ConditioningProvider(nn.Module):
    ...
    @property
    def con_conditions(self):
        return [k for k, v in self.conditioners.items() if isinstance(v, ChromaStemConditioner)]

    @property
    def has_con_condition(self):
        return len(self.wav_conditions) > 0
    

    @property
    def st_conditions(self):
        return [k for k, v in self.conditioners.items() if isinstance(v, style_wav_Conditioner)]

    @property
    def has_st_condition(self):
        return len(self.wav_conditions) > 0
    ...
    def tokenize(self, inputs: tp.List[ConditioningAttributes]) -> tp.Dict[str, tp.Any]:
        ...
        contents = self._collate_cons(inputs)
        styles = self._collate_styles(inputs)

        assert set(contents.keys() | styles.keys() ).issubset(set(self.conditioners.keys())), (
            f"Got an unexpected attribute! Expected {self.conditioners.keys()}, ",
            f"got {contents.keys(), styles.keys()}"
        )

        for attribute, batch in chain(contents.items(), styles.items()):
            output[attribute] = self.conditioners[attribute].tokenize(batch)
        return output

    def forward(self, tokenized: tp.Dict[str, tp.Any],style_type:str='ratio') -> tp.Dict[str, ConditionType]:
        ...
        if(style_type=='ratio'):
            ...
            return output
        elif(style_type=='last_step'):
            sp_output={}
            for attribute, inputs in tokenized.items():
                if(attribute=='self_st'):
                    condition, mask,sp_condition = self.conditioners[attribute](inputs)
                    output[attribute] = (condition, mask)
                    sp_output[attribute]=(sp_condition,mask)
                else:
                    condition, mask = self.conditioners[attribute](inputs)
                    output[attribute] = (condition, mask)
            return output,sp_output
    ...
    def _collate_cons(self, samples: tp.List[ConditioningAttributes]) -> tp.Dict[str, WavCondition]:
        """Generate a dict where the keys are attributes by which we fetch similar wavs,
        and the values are Tensors of wavs according to said attributes.
        ...
        *Note*: by the time the samples reach this function, each sample should have some waveform
        ...
        for sample in samples:
            for attribute in self.con_conditions:
                wav, length, sample_rate, path, seek_time = sample.content[attribute]
                assert wav.dim() == 3, f"Got wav with dim={wav.dim()}, but expected 3 [1, C, T]"
                assert wav.size(0) == 1, f"Got wav [B, C, T] with shape={wav.shape}, but expected B == 1"
                # mono-channel conditioning
                wav = wav.mean(1, keepdim=True)  # [1, 1, T]
    ...
    def _collate_styles(self, samples: tp.List[ConditioningAttributes]) -> tp.Dict[str, WavCondition]:
        """Generate a dict where the keys are attributes by which we fetch similar wavs,
        and the values are Tensors of wavs according to said attributes.
        ...
        *Note*: by the time the samples reach this function, each sample should have some waveform
        ...
        for sample in samples:
            for attribute in self.st_conditions:
                wav, length, sample_rate, path, seek_time = sample.style[attribute]
                assert wav.dim() == 3, f"Got wav with dim={wav.dim()}, but expected 3 [1, C, T]"
                assert wav.size(0) == 1, f"Got wav [B, C, T] with shape={wav.shape}, but expected B == 1"
                # mono-channel conditioning
                wav = wav.mean(1, keepdim=True)  # [1, 1, T]
...
class ConditionFuser(StreamingModule):
    ...
    def forward(
        ...
        '''in case if you have weight for cross attention.
        if(style_type == 'last_step'):
            sp_cross_attention_output = None
            for __,(sp_cond,sp_mask) in sp_condition_tensors.items():
                if sp_cross_attention_output is not None:
                    sp_cross_attention_output = torch.cat([sp_cross_attention_output, sp_cond], dim=1)
                else:
                    sp_cross_attention_output = sp_cond
            positions = torch.arange(
                sp_cross_attention_output.shape[1],
                device=sp_cross_attention_output.device
            ).view(1, -1, 1)
            pos_emb = create_sin_embedding(positions, sp_cross_attention_output.shape[-1])
            if(self.cross_attention_pos_emb_scale==0):self.cross_attention_pos_emb_scale=1.0
            sp_cross_attention_output = sp_cross_attention_output + self.cross_attention_pos_emb_scale * pos_emb'''
