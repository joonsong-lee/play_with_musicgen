...
class StreamingTransformer(StreamingModule):
    ...
    def forward(self, x: torch.Tensor,style_type:str,sp_input=False, *args, **kwargs):
        B, T, C = x.shape
        ...
        #there is no weight for cross-attention in pre-trained model. so we call forward twice and last stage input will be a fused logit with special condition
        elif(style_type=='last_step'):
            if(sp_input==True):
                x = self._apply_layer(self.layers[-1], x, *args, **kwargs)
            else:
                iter =1
                for layer in self.layers:
                    if(iter==len(self.layers)): break
                    else: x = self._apply_layer(layer, x, *args, **kwargs)
                    iter+=1
